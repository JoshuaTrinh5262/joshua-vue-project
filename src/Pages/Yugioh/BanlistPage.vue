...
export default defineComponent({
  ...
  setup() {
    const heading = ref("Yu-Gi-Oh Banlist");
    const subheading = ref("Yu-Gi-Oh Banlist page");
    ...
    const getYugiohBanlistsData = async (newPage, newPageSize) => {
      const result = await apiService.getYugiohBanlistsWithPaging(newPage, newPageSize, orderBy.value, orderDirection.value, search.value);
      if (!result.error) {
