...
export default defineComponent({
  ...
  setup() {
    const heading = ref("Yu-Gi-Oh Tournament");
    const subheading = ref("Yu-Gi-Oh Tournament page");
    ...
    const getYugiohTournamentsData = async (newPage, newPageSize) => {
      const result = await apiService.getYugiohTournamentsWithPaging(newPage, newPageSize, orderBy.value, orderDirection.value, search.value);
      if (!result.error) {
