...
export default defineComponent({
  ...
  setup() {
    const heading = ref("Yu-Gi-Oh Decklist");
    const subheading = ref("Yu-Gi-Oh Decklist page");
    ...
    const onSearch = (newSearchTerm) => {
      ...
      getYugiohDecklistsData(1, itemsPerPage.value);
    };

    const getYugiohDecklistsData = async (newPage, newPageSize) => {
      const result = await apiService.getYugiohDecklistsWithPaging(newPage, newPageSize, orderBy.value, orderDirection.value, search.value);
      if (!result.error) {
    ...
    const changePageSize = async (newPageSize) => {
      await getYugiohDecklistsData(1, newPageSize);
    };

    const changeCurrentPage = async (newPage) => {
      currentPage.value = newPage;
      await getYugiohDecklistsData(currentPage.value, itemsPerPage.value);
    };
