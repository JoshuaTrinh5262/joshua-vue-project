  ...
  <div>
    <page-title-component :heading="heading" :subheading="subheading" :icon="icon">
      <template v-slot:actions>
        ...
        <button @click="toggleExport" type="button" class="btn btn-info">
          Export
        </button>
      </template>
    ...
    <modal-component :title="isUpdateMode ? 'Update Dataset' : 'Add New Dataset'" :isOpen="showCreateModal"
      ...
      <template #body>
        ...
        <div class="position-relative form-group">
          ...
          <input name="category" id="category" placeholder="category" v-model="currentDataset.category" type="text"
            class="form-control" />
        </div>
      </template>
      <template #footer>
        <button type="text" class="btn btn-primary" @click="toggleModal">
          Cancel
        </button>
    ...
    <div class="main-card mb-3 card" v-if="showImport">
      ...
      <div class="card-body">
        <div class="form-inline">
          ...
          <div class="position-relative form-group">
            <button class="btn-success btn" @click="handleImport">
              Import
            </button>
          </div>
        </div>
      ...
      <div class="card-body">
        <div class="position-relative form-group">
          ...
          <div class="form-group">
            ...
            <input name="source_text_max_length" id="source_text_max_length" v-model="sourceTextMaxLength" type="number"
              placeholder="Source Text Max Length" class="form-control" />
          </div>
          <div class="form-group">
            <label for="target_text_max_length" class="">Target Text Max Length</label>
            <input name="target_text_max_length" id="target_text_max_length" v-model="targetTextMaxLength" type="number"
    ...
    <table-component :footer="true" :fields="fields" :items="items" custom-class="datase-table" @search="onSearchChange"
      @changeOrder="handleChangeOrder" @deleteRow="handleDeleteDataset" @updateRow="handleUpdateRow">
    </table-component>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleExport = async () => {
      try {
        var response = await apiService.exportDataset(
          fileName.value,
          sourceTextMaxLength.value,
          targetTextMaxLength.value
        );
