<template>
    <div>
        ...
        <table-component :footer=true :fields="fields" :items="items" custom-class="datase-table"
            @search="onSearchChange" @deleteRow="handleDeleteDataset" @updateRow="handleUpdateRow">
        </table-component>
...
export default defineComponent({
    ...
    setup() {
        ...
        const getDatasetData = async (newPage, newPageSize) => {
            const result = await apiService.getDatasetUnprocess(newPage, newPageSize, orderBy.value, orderDirection.value, search.value);
            const count = await apiService.getCountDatasetUnprocess(search.value);
        ...
        const updateDataset = async () => {
            try {
                await apiService.updateDatasetUnprocess(currentDataset);
                cleanCurrentDataset();
                toggleModal();
                notification.value = {
                    title: "Success",
        ...
        const handleDeleteDataset = async (id) => {
            if (confirm(`Are you sure you want to delete dataset ${id}?`)) {
                try {
                    await apiService.deleteDatasetUnprocess(id);
                    notification.value = {
                        title: "Success",
