...
export const getDiscographiesWithPaging = async (
    ...
    try {
        ...
        const discographies = data.map((discography) => ({
            ...
            talents: discography.discography_talent?.map((et) => et.talent?.name) || [],
        }));
...
export const createDiscography = async (discography, selectedTalents) => {
    try {
        const { data: discographyData, error: discographyError } = await supabase
            .from("discography")
            .insert(discography)
            .select("*")
            .single();
    ...
    try {
        ...
        if (talentsToInsert.length > 0) {
            const { error: insertError } = await supabase
                .from("discography_talent")
                .insert(talentsToInsert.map(talentId => ({ discography_id: discographyId, talent_id: talentId })));
