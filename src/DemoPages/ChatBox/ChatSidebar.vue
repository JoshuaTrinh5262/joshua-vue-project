<template>
  <div class="h-auto">
    ...
    <div class="h-auto space-4 overflow-y-auto border-b border-zinc-300 px-2 py-4 dark-border-zinc-700">
      <button :key="chat.id" v-for="chat in chatGroups" @click="setCurrentChat(chat.id)" :class="{'dark-bg-zinc-800 bg-zinc-200': currentChat?.id == chat.id}" class="btn btn-outline-success d-flex w-100 flex-column gap-2 rounded-lg px-3 py-2 text-left transition-colors duration-200 hover-bg-zinc-200 dark-hover-bg-zinc-800 focus-outline-none focus-ring-2 focus-ring-blue-500 dark-bg-zinc-700 dark-text-zinc-200 dark-placeholder-zinc-400 dark-focus-ring-blue-500">
        <span class="text-sm font-weight-medium capitalize text-zinc-700 dark-text-zinc-200">{{ chat.id}} {{ chat.chat_group_name }}</span>
